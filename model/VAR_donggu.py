...
donggu_data: pd.DataFrame = data["동구"]

# '시점' 열을 datetime 형식으로 변환
donggu_data["시점"] = pd.to_datetime(donggu_data["시점"], format="%Y.%m.%d")
donggu_data.set_index("시점", inplace=True)
donggu_data.index.freq = "MS"  # 명시적으로 빈도 설정

# 필요한 열만 선택
variables = ["인구수", "세대수", "사업체수", "주택거래량", "아파트거래량", "토지거래량"]
donggu_data = donggu_data[variables]

# 데이터 전처리: 결측치 제거 및 차분
donggu_data = donggu_data.dropna()
donggu_data_diff = donggu_data.diff().dropna()

# VAR 모델 학습
model = VAR(donggu_data_diff)
model_fit = model.fit(maxlags=15, ic="aic")

# 예측
forecast_steps = 36  # 예측할 기간 설정 (예: 3년)
lag_order = model_fit.k_ar
forecast_input = donggu_data_diff.values[-lag_order:]

forecast_diff = model_fit.forecast(y=forecast_input, steps=forecast_steps)
forecast_diff_df = pd.DataFrame(
    forecast_diff,
    index=pd.date_range(
        start=donggu_data.index[-1], periods=forecast_steps + 1, freq="MS"
    )[1:],
    columns=variables,
)

# 차분 역변환
last_observation = donggu_data.iloc[-1]
...
file_path = "data/donggu_predict.xlsx"
with pd.ExcelWriter(file_path, engine="openpyxl") as writer:
    forecast_copy.to_excel(writer, sheet_name="동구", index=False)


# 결과 시각화
plt.figure(figsize=(12, 6))
plt.title("예측:동구")
plt.plot(donggu_data["인구수"], label="실제 인구수:동구", color="blue")
plt.plot(forecast["인구수"], label="예측 인구수:동구", color="red")
plt.plot(donggu_data["세대수"], label="실제 세대수:동구", color="green")
plt.plot(forecast["세대수"], label="예측 세대수:동구", color="red")
plt.legend()
plt.show()
...
train_data = donggu_data[:split_date]
test_data = donggu_data[split_date:]
